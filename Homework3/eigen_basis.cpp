//Le Nguyen
//nguye639@msu.edu
//03-May-2019 got assistance on program

/* Remark: I was unequiped to do this problem with my current level of physics, and needed a large amount of assistance on this code and understanding what it did. If my code looks similar to my peers, it is, and I understood it to the best of my capacity, but this is not an orginial solution 

B was chosen to be a value that  minimized the error when plotted with the actual ground state function. This was determined by looking at the plots. As the dimension got larger, the function converged to the actual ground state wavefunction. B was found to be  approximatley 1.9 to fitthe ground state wavefunction.
...
main ()
{
  ...
  for (int i = 0; i < dimension; i++)
    ...
    ofstream out ("eigen_basis20.dat");   // open the output file
    double u = 0;
    double denom= b_ho*b_ho; //normalize constant
    for (double r = 0; r<15; r+=0.01){
      for (int j=0; j < dimension; j++){
      ho_parameters.i=0;
      ho_parameters.j= j;

    u+=-ho_radial (1, 0, b_ho, r)*gsl_vector_get (Eigval_ptr, 0);

    out << scientific << setprecision (8)
      << r << "   "
                        << u * pow(1/denom,.25)<< endl;
      u = 0;
